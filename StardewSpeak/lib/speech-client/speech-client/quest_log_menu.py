...
mapping = {
    ...
    '(item | quest) <positive_index>': df_utils.async_action(focus_quest, 'positive_index'),
}

def load_grammar():
    extras=[df_utils.positive_index]
    grammar = menu_utils.build_menu_grammar("quest_log", mapping, QUEST_LOG_MENU, extras=extras)
    grammar.load()